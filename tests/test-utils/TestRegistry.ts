...
export class MockMetricsCollector {
  public readonly metrics: Array<{ name: string; value: number; labels?: Record<string, string> }> = [];
...
export async function createTestRegistry(
  ...
  registry.override('metricsCollector', mockMetrics as unknown as ServiceDefinitions['metricsCollector']);
  registry.override('healthCheck', mockHealthCheck as unknown as ServiceDefinitions['healthCheck']);
...
export async function createTestDatabase(): Promise<{
  ...
  const { MigrationRunner } = await import('../../src/layers/l1-persistence/MigrationRunner');
