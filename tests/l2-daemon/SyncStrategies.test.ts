...
import {
  SyncContext,
  SyncResult,
} from '../../src/layers/l2-daemon/sync-engine/sync/SyncStrategy';
import { TaskSyncStrategy } from '../../src/layers/l2-daemon/sync-engine/sync/TaskSyncStrategy';
import { AnnouncementSyncStrategy } from '../../src/layers/l2-daemon/sync-engine/sync/AnnouncementSyncStrategy';
...
describe('SyncStrategies', () => {
  ...
  describe('TaskSyncStrategy', () => {
    ...
    it('should log diagnostics when enabled', async () => {
      ...
      // Mock different queries: course lookup returns course with auto-accept, task lookup returns null
      mockDb.executeReadOne.mockImplementation((sql: string) => {
        if (sql.includes('FROM courses')) {
          return { name: 'Test Course', auto_accept_canvas_tasks: 1 }; // AUTO_ACCEPT_ALL
        }
    ...
    it('should auto-complete graded tasks', async () => {
      ...
      const autoCompleteCalls = mockDb.executeWrite.mock.calls.filter((call) =>
        call[0].includes('SET is_completed = 1')
      );
  ...
  describe('AnnouncementSyncStrategy', () => {
    ...
    it('should request only_announcements from discussion_topics endpoint', async () => {
      ...
      expect(mockClient.getAll).toHaveBeenCalledWith('/courses/123/discussion_topics', {
  ...
  describe('Transaction handling', () => {
    ...
    it('should handle transaction errors', async () => {
      mockClient.getAll.mockResolvedValueOnce([{ id: 1, name: 'Task 1', due_at: null }]);
      mockDb.executeReadOne.mockReturnValue({ name: 'Course' });
