...
import { HtmlDependencyResolver, HtmlSourceType } from '../../src/layers/l2-daemon/HtmlDependencyResolver';
...
describe('HtmlDependencyResolver', () => {
  ...
  describe('cycle detection', () => {
    it('detects self-reference cycle', () => {
      ...
      mockDb.executeReadOne.mockImplementation(createMockImplementation((sql, params) => {
  ...
  describe('shared files (Option B)', () => {
    it('reuses existing download location for shared files', () => {
      ...
      mockDb.executeReadOne.mockImplementation(createMockImplementation((sql, params) => {
        if (sql.includes('course_pages') && (params[0] === 'page1' || params[1] === 'page1')) {
          return { body_html: html1 };
        }
  ...
  describe('source type handling', () => {
    it('resolves assignment HTML', () => {
      ...
      const result = resolver.resolve('assignment', 'assign1', '123', '/test/assign1.html');
    ...
    it('resolves syllabus HTML', () => {
      ...
      const result = resolver.resolve('syllabus', 'syllabus', '123', '/test/syllabus.html');
    ...
    it('calculates missingCount correctly', () => {
      ...
      mockDb.executeReadOne.mockImplementation(createMockImplementation((sql, params) => {
        ...
        if (sql.includes('resources')) {
          const id = params[0];
          if (id === '1') {
            return { id: 1, local_path: '/downloaded.png', title: 'img1.png', first_referenced_by: 'page:other' };
          }
          if (id === '2') {
            return { id: 2, local_path: null, title: 'img2.png', first_referenced_by: null };
          }
          if (id === '3') {
            return { id: 3, local_path: null, title: 'img3.png', first_referenced_by: null };
          }
        }
        return null;
      }));
  ...
  describe('areAllDependenciesDownloaded', () => {
    it('returns true when all dependencies are downloaded', () => {
      ...
      mockDb.executeReadOne.mockImplementation(createMockImplementation((sql, params) => {
        ...
        if (sql.includes('resources') && params[0] === '1') {
          return { id: 1, local_path: '/downloaded.png', title: 'img.png', first_referenced_by: 'page:page1' };
        }
        return null;
      }));
    ...
    it('returns false when some dependencies are missing', () => {
      ...
      mockDb.executeReadOne.mockImplementation(createMockImplementation((sql, params) => {
        ...
        if (sql.includes('resources') && params[0] === '1') {
          return { id: 1, local_path: null, title: 'img.png', first_referenced_by: null };
        }
        return null;
      }));
