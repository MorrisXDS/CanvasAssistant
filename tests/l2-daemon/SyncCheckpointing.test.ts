...
jest.mock('../../src/layers/l2-daemon/CanvasClient', () => ({
  CanvasClient: jest.fn().mockImplementation(() => mockClient),
}));

jest.mock('../../src/layers/l2-daemon/RateLimiter', () => ({
  RateLimiter: jest.fn().mockImplementation(() => mockRateLimiter),
}));

jest.mock('../../src/layers/l2-daemon/SyncConflictResolver', () => ({
...
jest.mock('../../src/layers/l2-daemon/HtmlFileExtractor', () => ({
  HtmlFileExtractor: jest.fn().mockImplementation(() => ({
    extract: jest.fn().mockReturnValue([]),
  })),
}));

import { SyncEngine, SyncCheckpoint, SyncOptions } from '../../src/layers/l2-daemon/SyncEngine';
...
describe('SyncCheckpointing', () => {
  ...
  describe('checkpoint error handling', () => {
    ...
    it('should preserve checkpoint data on partial failure', async () => {
      ...
      const updateCalls = mockDb.executeWrite.mock.calls.filter(
        (call) => call[0].includes('UPDATE sync_checkpoints')
      );
