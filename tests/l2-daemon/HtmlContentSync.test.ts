...
import {
  ...
  ExtractedResource,
  HtmlContentItem,
} from '../../src/layers/l2-daemon/HtmlContentSync';
...
describe('HtmlContentSync', () => {
  ...
  describe('createDownloadRequests', () => {
    ...
    it('should skip images when downloadImages is disabled', () => {
      ...
      const resources: ExtractedResource[] = [
        { originalUrl: '/image.png', type: 'image', filename: 'image.png', attribute: 'src' },
      ];
    ...
    it('should convert relative URLs to absolute', () => {
      const resources: ExtractedResource[] = [
        { originalUrl: '/images/photo.jpg', type: 'image', filename: 'photo.jpg', attribute: 'src' },
      ];
  ...
  describe('syncCourseHtmlContent', () => {
    ...
    it('should collect pages, announcements, and assignments', async () => {
      mockDb.executeRead.mockImplementation((query: string) => {
        if (query.includes('FROM course_pages')) {
          return [{ external_id: 'page-1', title: 'Page 1', body_html: '<p>Page</p>', module_id: null }];
        }
        ...
        if (query.includes('FROM tasks')) {
          return [
            { external_id: 'task-1', title: 'Task 1', description: '<p>Task</p>', module_id: null },
          ];
        }
        return [];
      });
    ...
    it('should resolve module names', async () => {
      mockDb.executeRead.mockImplementation((query: string) => {
        if (query.includes('FROM course_pages')) {
          return [{ external_id: 'page-1', title: 'Page 1', body_html: '<p>Page</p>', module_id: 5 }];
        }
        return [];
      });
      mockDb.executeReadOne.mockImplementation((query: string) => {
