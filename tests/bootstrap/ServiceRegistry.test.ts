...
import type {
  ...
  ServiceToken,
} from '../../src/layers/bootstrap/ServiceTokens';
...
type AnyRegistry = ServiceRegistry & {
  register: (token: string, factory: () => any) => any;
  get: (token: string) => any;
  override: (token: string, instance: any) => any;
};
...
describe('ServiceRegistry', () => {
  ...
  describe('Shutdown', () => {
    it('should call stop on services in reverse order', async () => {
      ...
      service1.stop = jest.fn(() => {
        callOrder.push('service1');
      });
      service2.stop = jest.fn(() => {
