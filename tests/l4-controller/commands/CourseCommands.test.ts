...
import { UseGraceTokenCommand } from '../../../src/layers/l4-controller/commands/UseGraceTokenCommand';
import {
  CommandContext,
...
describe('Course Commands', () => {
  ...
  describe('UseGraceTokenCommand', () => {
    let command: UseGraceTokenCommand;
    let testTaskId: number;
    let policyId: number;

    beforeEach(() => {
      command = new UseGraceTokenCommand();
      ...
      const policyResult = db.executeWrite(
        `INSERT INTO course_policies (course_id, policy_type, policy_name, policy_config, is_user_verified, is_active)
         VALUES (?, 'grace_tokens', 'Grace Tokens', ?, 1, 1)`,
        [testCourseId, policyConfig],
        'course_policies'
      );
      policyId = policyResult.lastInsertRowid as number;
    });
    ...
    describe('execute', () => {
      it('should use grace token successfully', async () => {
        const result = await command.execute(context, {
          courseId: testCourseId,
          taskId: testTaskId,
          tokensToUse: 1,
        ...
        expect(result.data?.remainingTokens).toBe(2);
        expect(result.data?.hoursExtended).toBe(24);
        expect(result.data?.newDeadline).toBeInstanceOf(Date);
        ...
        expect(config.tokens_used).toBe(1);
      });

      it('should fail when no grace token policy exists', async () => {
        // Delete the policy
        db.executeWrite(
        ...
        expect(result.success).toBe(false);
        expect(result.error).toContain('No grace token policy');
      });
      ...
      it('should fail when task not in specified course', async () => {
        // Create another course
        const otherCourseResult = db.executeWrite(
          `INSERT INTO courses (external_id, code, name, target_grade)
           VALUES ('other_course', 'OTHER101', 'Other Course', 80)`,
          [],
          'courses'
        );
        const otherCourseId = otherCourseResult.lastInsertRowid as number;

        // Try to use token with wrong course
        const result = await command.execute(context, {
        ...
        expect(result.success).toBe(false);
        expect(result.error).toContain('Task not found in this course');
      });

      it('should fail when exceeding max tokens per task', async () => {
        const result = await command.execute(context, {
          courseId: testCourseId,
          taskId: testTaskId,
          tokensToUse: 3, // max is 2
        });

        expect(result.success).toBe(false);
        expect(result.error).toContain('Cannot use more than');
      });

      it('should fail when not enough tokens available', async () => {
        // Use up all tokens
        db.executeWrite(
          `UPDATE course_policies SET policy_config = ? WHERE id = ?`,
          [
            JSON.stringify({
              type: 'grace_tokens',
              total_tokens: 3,
      ...
      it('should fail for excluded task types', async () => {
        // Create an exam task
        const examResult = db.executeWrite(
          `INSERT INTO tasks (external_id, source_type, course_id, title, weight, due_at, is_completed)
           VALUES ('exam_1', 'canvas', ?, 'Final Exam', 30, datetime('now', '+1 day'), 0)`,
          [testCourseId],
          'tasks'
        );
        const examTaskId = examResult.lastInsertRowid as number;
        ...
        expect(result.success).toBe(false);
        expect(result.error).toContain('cannot be used for this task type');
      });
    });
  });
});
