...
import { UpdateTargetGradeCommand } from '../../../src/layers/l4-controller/commands/UpdateTargetGradeCommand';
import { SimulateGradeCommand } from '../../../src/layers/l4-controller/commands/SimulateGradeCommand';
import { ClearSimulationCommand } from '../../../src/layers/l4-controller/commands/ClearSimulationCommand';
import {
  CommandContext,
  ...
  describe('UpdateTargetGradeCommand', () => {
    ...
    describe('execute', () => {
      ...
      it('should mark field as locally modified', async () => {
        ...
        // Check if field was marked as modified
        const modified = db.executeReadOne<{ id: number }>(
          `SELECT id FROM course_local_modifications
           WHERE course_id = ? AND field_name = 'target_grade'`,
          [testCourseId]
        );
  ...
  describe('SimulateGradeCommand', () => {
    ...
    describe('validate', () => {
      it('should reject invalid task ID', () => {
        const result = command.validate({ taskId: 0, simulatedGrade: 85 });
        expect(result.valid).toBe(false);
        expect(result.error).toContain('Invalid task ID');
      });

      it('should reject grade below 0', () => {
        const result = command.validate({ taskId: 1, simulatedGrade: -5 });
        expect(result.valid).toBe(false);
        expect(result.error).toContain('between 0 and 100');
      });

      it('should reject grade above 100', () => {
        const result = command.validate({ taskId: 1, simulatedGrade: 150 });
        expect(result.valid).toBe(false);
        expect(result.error).toContain('between 0 and 100');
      });

      it('should accept valid params', () => {
        const result = command.validate({ taskId: 1, simulatedGrade: 85 });
        expect(result.valid).toBe(true);
      });
    });
