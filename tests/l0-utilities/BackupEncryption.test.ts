...
describe('BackupEncryption', () => {
  ...
  describe('file format verification', () => {
    it('should create encrypted file with correct structure: salt(32) + iv(16) + authTag(16) + data', () => {
      ...
      expect(encryptedData.length).toBeGreaterThanOrEqual(originalData.length + headerSize);
      ...
      const authTag = encryptedData.subarray(saltSize + ivSize, saltSize + ivSize + authTagSize);
