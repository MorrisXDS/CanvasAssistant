...
import {
  ...
  HealthStatus,
  HealthReport,
} from '../../src/layers/l0-utilities/HealthCheck';
...
describe('HealthCheck', () => {
  ...
  describe('registerProbe', () => {
    ...
    it('should allow recommended actions', async () => {
      ...
      expect(report.recommendations.some((r) => r.includes('Restart the service'))).toBe(
        true
      );
    });
  });
  ...
  describe('unregisterProbe', () => {
    ...
    it('should remove a registered probe', async () => {
      ...
      await healthCheck.runChecks();
      expect(
        healthCheck.getStatus().probes.some((p) => p.name === 'removable-probe')
      ).toBe(true);
  ...
  describe('overall status calculation', () => {
    beforeEach(() => {
      healthCheck = new HealthCheck({
        ...
        runOnStartup: false,
        logger,
        // Use high thresholds so built-in probes always return 'healthy' during tests
        thresholds: { memoryMb: 4096 },
      });
    });
  ...
  describe('getStatus', () => {
    ...
    it('should return updated report after checks', async () => {
      // Unregister built-in probes so only our custom probe affects the result
      // (built-in memory probe can return 'unhealthy' under memory pressure)
      healthCheck.unregisterProbe('memory');
      healthCheck.unregisterProbe('process');
      ...
      // Recommendations include probe name prefix
      expect(
        report.recommendations.some((r) => r.includes('Check system resources'))
      ).toBe(true);
    });
  });
