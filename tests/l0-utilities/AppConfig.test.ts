...
describe('AppConfig', () => {
  ...
  describe('validate', () => {
    ...
    it('should detect invalid urgency curve ordering', () => {
      ...
      const result = config.validate();
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('urgencyCurve.criticalHours must be less than highHours');
    });
  });
  ...
  describe('DEFAULT_APP_CONFIG', () => {
    ...
    it('should have three refresh tiers', () => {
      ...
      expect(DEFAULT_APP_CONFIG.intelligence.priority.refreshTiers[1].name).toBe('sixHourly');
      expect(DEFAULT_APP_CONFIG.intelligence.priority.refreshTiers[2].name).toBe('halfHourly');
    });
  });
