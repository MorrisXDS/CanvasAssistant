...
describeIfAvailable('node:sqlite Stability Tests', () => {
  ...
  describe('3. Prepared Statements', () => {
    ...
    it('UPDATE returns .changes reflecting rows modified', () => {
      ...
      const result = db
        .prepare('UPDATE items SET value = ? WHERE name = ?')
        .run(999, 'item1');
      expect(result.changes).toBe(1);
    ...
    it('supports positional ? placeholders', () => {
      ...
      const row = db
        .prepare('SELECT * FROM items WHERE name = ? AND value = ?')
        .get('positional', 42);
      expect(row).toBeDefined();
    ...
    it('supports named parameters with :key syntax', () => {
      ...
      const row = db
        .prepare('SELECT * FROM items WHERE name = :name')
        .get({ ':name': 'named' });
      expect(row).toBeDefined();
  ...
  describe('4. Raw SQL Execution', () => {
    ...
    it('exec() handles ALTER TABLE', () => {
      ...
      db.prepare('INSERT INTO t (id, name, email) VALUES (?, ?, ?)').run(
        1,
        'test',
        'a@b.com'
      );
  ...
  describe('8. Data Types & Edge Cases', () => {
    ...
    it('TEXT with UTF-8 (emoji, CJK, Cyrillic)', () => {
      ...
      for (let i = 0; i < testStrings.length; i++) {
        db.prepare('INSERT INTO types (id, text_val) VALUES (?, ?)').run(
          i + 1,
          testStrings[i]
        );
      }
  ...
  describe('9. Error Handling', () => {
    ...
    it('UNIQUE constraint violation throws', () => {
      ...
      db.prepare('INSERT INTO child (id, parent_id, name) VALUES (?, ?, ?)').run(
        1,
        1,
        'unique_name'
      );
  ...
  describe('10. Performance Benchmarks', () => {
    it('1000 INSERTs in transaction: measure ops/sec', () => {
      ...
      console.log(
        `  INSERT: ${elapsed.toFixed(2)}ms for 1000 rows (${opsPerSec.toLocaleString()} ops/sec)`
      );
    ...
    it('1000 SELECTs: measure ops/sec', () => {
      ...
      console.log(
        `  SELECT: ${elapsed.toFixed(2)}ms for 1000 reads (${opsPerSec.toLocaleString()} ops/sec)`
      );
      expect(elapsed).toBeLessThan(5000);
      db.close();
    });
