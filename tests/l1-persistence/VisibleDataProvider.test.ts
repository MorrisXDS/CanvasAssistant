...
describe('VisibleDataProvider', () => {
  ...
  describe('Term Selection', () => {
    ...
    test('setTermSelection invalidates cache', () => {
      ...
      // Cache should be invalidated (but result should be same since no term filter)
  ...
  describe('getVisibleCourseIds', () => {
    ...
    test('auto mode filters by active terms', () => {
      ...
      seedEnrollmentTerm(db, { external_id: '100', name: 'Current Semester', end_at: futureDate.toISOString() });
      seedEnrollmentTerm(db, { external_id: '200', name: 'Past Semester', end_at: pastDate.toISOString() });
...
function seedCourse(db: Database, data: {
  id: number;
  is_hidden?: number;
...
function seedTask(db: Database, data: {
  id: number;
  course_id: number;
  title: string;
  is_completed?: number;
...
function seedEnrollmentTerm(db: Database, data: {
  external_id: string;
  name: string;
  end_at: string;
}) {
