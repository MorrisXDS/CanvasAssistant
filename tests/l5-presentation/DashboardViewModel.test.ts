  ...
  describe('computeDashboardViewModel', () => {
    ...
    describe('priority queue', () => {
      ...
      it('sorts by due date ascending (earliest first)', () => {
        ...
        laterDue.setDate(laterDue.getDate() + 7);
        const earlierDue = new Date();
        earlierDue.setDate(earlierDue.getDate() + 1);
        const middleDue = new Date();
        middleDue.setDate(middleDue.getDate() + 4);

        const task1 = createTask({ id: 1, dueAt: laterDue.toISOString() });
        const task2 = createTask({ id: 2, dueAt: earlierDue.toISOString() });
        const task3 = createTask({ id: 3, dueAt: middleDue.toISOString() });
        ...
        expect(viewModel.priorityQueue[0].task.id).toBe(2); // Earliest due
        expect(viewModel.priorityQueue[1].task.id).toBe(3); // Middle due
        expect(viewModel.priorityQueue[2].task.id).toBe(1); // Latest due
      });
      ...
      it('calculates urgency level for tasks due within 3 days', () => {
        const course = createCourse();
        const twoDays = new Date();
        twoDays.setDate(twoDays.getDate() + 2);

        const highUrgencyTask = createTask({
          id: 1,
