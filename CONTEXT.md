...
**⚠️ Migration-runner FK-OFF blocker (the gate on the last two drops):** `MigrationRunner.runMigration` runs each migration inside `db.transaction()`, and `PRAGMA foreign_keys` is a no-op inside a transaction (FK enforcement stays ON during migrations). So a migration **cannot** rebuild any table that has child FK references (`notifications` ← attachments/file-refs; `tasks` ← many) — dropping the old table mid-rebuild fails the FK check. Dropping `course_policies`/`course_task_groups` both require such a rebuild, so they're **blocked on first adding FK-OFF support to the migration engine** (a flagged migration that runs outside the transaction with `foreign_keys=OFF` + a `foreign_key_check` after). Tracked in [docs/FOLLOWUPS.md](docs/FOLLOWUPS.md).
