...
The `course_policies` table was designed to record per-course grading policies (e.g. "drop the lowest quiz", "10% per day late") extracted from announcements / syllabi by L3 intelligence. Per [ADR-0003](docs/adr/0003-removal-of-l3-intelligence-layer.md) that subsystem was removed; the table never had a live INSERT writer. The whole family is now gone: `grade_replacements`/`weight_transfers` (migration 105), `policy_rules`/`grace_tokens`/`grace_token_usage` (109), and `course_policies` itself (110 — the first ADR-0009 FK-off rebuild, which dropped `notifications.linked_policy_id` first). `MarkSyllabusReviewedCommand`'s dead `course_policies` UPDATE was removed (its real work — `course_syllabuses.last_reviewed_at` — is untouched), as were the export/import policy branches. The `Policy` Zod schema/type, `PolicyRow`/`PolicyRowMinimal`, and `mapPolicyRowToEntity` were removed 2026-06-02 (they had no remaining consumers). Only `course_task_groups` remains of the policy/grouping zombies (deliberately kept — see the zombie inventory).
...
- `course_task_groups` — legacy "TaskGroup" table; dead (no writers/readers), `tasks.task_group_id` is always NULL. **Deliberately LEFT in place (2026-06-02) — accepted-dead, not a TODO.** It's the last zombie table, but dropping it is the only cleanup whose cost exceeds its benefit: it requires rebuilding the central `tasks` table (~54 cols / 16 indexes / 5 FKs incl. a self-ref / 5 child tables — the riskiest migration in the repo) AND tearing the `taskGroupId` field out of the IPC contract + ~10 files of plumbing — all to delete one empty table + an always-null column. No bug, no feature, no harm in keeping it. The FK-off capability (ADR-0009) makes it _possible_ if a provably zombie-free schema is ever wanted; it's just not worth the blast radius now. (`canvas_assignment_groups` is the live grouping table.)

**✅ Migration-runner FK-OFF support — landed [ADR-0009](docs/adr/0009-migration-fk-off-rebuilds.md) (2026-06-02).** A migration may set `disableForeignKeys: true` to run with `foreign_keys=OFF` (toggled outside the transaction, restored in a `finally`) + a `PRAGMA foreign_key_check` after the body. First (and so far only) use: migration 110 (course_policies). The schema-zombie cleanup is otherwise **complete** — `course_task_groups` is the one table deliberately left (see above).
