...
- `course_policies` — only an UPDATE writer (`MarkSyllabusReviewedCommand`), no INSERT. Dead read chain (`PolicyReader` + handlers) removed 2026-06-01. **The TABLE can't be dropped yet** — `notifications.linked_policy_id` FKs it, and SQLite errors `no such table: course_policies` on _every_ `notifications` INSERT if the table is dropped while that FK exists (even with all values NULL). Removing the FK needs a `notifications` rebuild — see the migration-runner blocker below.
- `course_task_groups` — legacy "TaskGroup" table; dead (no writers/readers) BUT `tasks.task_group_id` + `course_policies.target_group_id` FK to it. Needs a `tasks` rebuild to drop those columns first.

**✅ Migration-runner FK-OFF support (was the gate on the last two drops) — landed [ADR-0009](docs/adr/0009-migration-fk-off-rebuilds.md) (2026-06-02):** `MigrationRunner.runMigration` runs each migration inside `db.transaction()` where `PRAGMA foreign_keys` is a no-op, so an unflagged migration **cannot** rebuild a table with child FK references (dropping the old table mid-rebuild fails the FK check). A migration may now set `disableForeignKeys: true` to run with `foreign_keys=OFF` (toggled outside the transaction, restored in a `finally`) + a `PRAGMA foreign_key_check` after the body that aborts a rebuild leaving dangling refs. With that capability, dropping `course_policies` (needs a `notifications` rebuild) and `course_task_groups` (needs a `tasks` rebuild) is now **unblocked** — each just needs writing as a flagged rebuild migration.
